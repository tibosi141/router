<template>
  <div class="page">
    ...
    <div id="main"></div>
    <button @click="getUser('王嘉然')">测试1</button>
  </div>
</template>
...
import { userAPI } from '@/assets/apis/user'

const router = useRouter()

const getUser = async (name: string) => {
  const res = await userAPI({ name })
}
const toDetail = (id: number) => router.push({ name: 'Detail', params: { id } })
const initChart = () => {
  // 数据
  const chartData = [
    {
